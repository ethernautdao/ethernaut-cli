...
describe('infura-key task', function () {
  ...
  beforeEach(function () {
    ...
    originalEnv = process.env.INFURA_API_KEY
  ...
  afterEach(function () {
    ...
    process.env.INFURA_API_KEY = originalEnv
  ...
  it('should set the Infura API Key and instruct to restart', async function () {
    const testKey = 'TEST_API_KEY'

    const result = await hre.run(
      { scope: 'optigov', task: 'infura-key' },
      { apiKey: testKey },
    )

    // Verify the output contains the new key and the restart message.
    assert(result.includes(`- Infura API Key set to ${testKey}`))
    assert(
      result.includes(
    ...
    assert.strictEqual(process.env.INFURA_API_KEY, testKey)
  })
})
